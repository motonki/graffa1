...
vector<Vertex> loadUserGeneratedModel() {
	...
	for(auto i = 0u; i < faces; ++i)	{
		...
		v1.position = Vec3f(0.25*FW::cos(i*FW_PI / 20), -1, 0.25*FW::sin(i*FW_PI / 20));
		v2.position = Vec3f(0.25*FW::cos((i+1)*FW_PI / 20), -1, 0.25*FW::sin((i + 1)*FW_PI / 20));

		//Vec3f n1 = v1.position.normalized();
		//Vec3f n2 = v2.position.normalized();
		//Vec3f n = cross(n1,n2);

		Vec3f n = cross(v1.position, v2.position);
		n.normalize();
